...
Make `layers/meta-fluorite-trial` in the `meta-fluorite-trial` repository the source of truth for Fluorite-specific recipes, packagegroups, patches, runtime configuration, and image additions. Use only the fixed upstream Flutter layer for the `flutter-app` class; do not require the historical external `self-install/meta-flutter-apps` layer.

## Facts

- The existing Fluorite demo recipe lived in `self-install/meta-flutter-apps` while project-owned scene patches lived in the imported legacy `meta-local` tree but were not listed in effective `SRC_URI`.
...
- Add the demo recipe and Fluorite packagegroup to `layers/meta-fluorite-trial`.
...
- First gate: `bitbake-layers show-layers` must show `meta-fluorite-trial` and `external/meta-flutter`, without `self-install/meta-flutter-apps`.
...
- The pre-rename Mini PC trial build listed the project layer, fixed `external/meta-flutter`, and `vulkan-layer`; the historical `self-install/meta-flutter-apps` layer was absent. The layer is now renamed to `meta-fluorite-trial` and must be revalidated after synchronization.
